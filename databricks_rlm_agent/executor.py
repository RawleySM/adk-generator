...
Local Mode Support:
    When ADK_RUN_MODE=local, the executor injects execute_sql() instead of
    spark into the execution globals, allowing code to query UC data via
    SQL Warehouse API without requiring a SparkSession.
...
def execute_artifact(
    ...
    Args:
        spark: SparkSession for execution context (None in local mode).
    ...
    try:
        ...
        # Inject appropriate data access method based on run mode
        if run_mode == "local":
            # In local mode, inject execute_sql for UC queries via SQL Warehouse API
            from databricks_rlm_agent.sql_warehouse import execute_sql
            exec_globals["execute_sql"] = execute_sql
            logger.info("Local mode: injected execute_sql() for UC queries")
        else:
            # In Databricks mode, inject SparkSession
